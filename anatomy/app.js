...
const HISTORY_KEY = "j-anatomy-kokushi-history-v1";
let questions = [];
let queue = [];
let position = 0;
...
function renderExplanation(question) {
  const detail = window.anatomyExplanationFor
    ? window.anatomyExplanationFor(question)
    : {why: question.explanation};
  const blocks = [
    [detail.whyLabel || "なぜこの解答？", detail.why],
    ["何と関係する？", detail.relation],
    ["見分け方", detail.keyPoint],
  ].filter(([, text]) => text);
